...
type TypeOfAddPointsParams = [
  personalityTypes: TypeOfPersonalityPointsIndex,
  score: number
];
    ...
    <B.FirstView>
      ...
      {choicesArray.map(value => (
        <QuestionsLayout
          ...
          choiceKey={value[0]}
          pageNumber={pageNumber}></QuestionsLayout>
      ))}
    </B.FirstView>
  );
}
  ...
  const addPoints = (id: string) => {
    choicesArray.forEach(eachChoicesArray => {
      if (eachChoicesArray[0] === id) {
        ...
        yes.forEach(value => {
          questionPageLogic.addPoints(
            value[0] as TypeOfPersonalityPointsIndex,
            value[1] as number
          );
        });
      }
